  ...
  <div class="product-more">
    <el-row>
      ...
      <el-col :span="20" class="more-center">
        ...
        <el-col :span="22" :offset="1" class="container-k2">
          <div class="swiper-container1" ref="mySwiper2"><!--:style="{height:screenWidth/24*22*0.36+'px'}" 当swiper为column时需要高度设置-->
            <div class="swiper-wrapper">
              ...
              @mouseleave="leave"
               ...
               :style="{height:screenWidth/24*22*0.2+'px'}">
...
export default {
  ...
  mounted() {
    ...
    axios.get('/portalProducts/getListByFirstType',{params:{
      ...
      for(var i=0;i<res.data.content.length;i++){
        ...
        that.Sortedarray1=that.Sortedarray1.concat(res.data.content[i].list);
      }
    }).catch(err=>{
      console.log(err)
    })
   that.theswiper=function(){ setTimeout(function(){
      var mySwiper2 = new Swiper(".swiper-container1", {
      grabCursor: true,
      roundLengths: true,//宽高四舍五入
      watchOverflow: true,//当没有足够的slide切换时，例如只有1个slide，swiper会失效
      observer: true,
      spaceBetween: 20,
      slidesPerView: 4,
      slidesPerColumn: 2,
  ...
  methods: {
    productchange(id) {
      this.num = id;
      if(id==-1){
        this.Sortedarray=this.Sortedarray1;
      }else{
        this.Sortedarray=this.morelist[id].list;
      }
      this.theswiper()
    },
    over(id) {
